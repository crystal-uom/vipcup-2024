...
if __name__ == '__main__':
    ...
    image_path = "D:/VIP Cup/Dataset/ICIP training data/0/RawDataQA (1)/RawDataQA1 (21).tiff"
    #bm3d_img = os.path.join(os.path.dirname(image_path),"bm3d.tiff")
    bm3d_img = "D:/VIP Cup/Code/BM3D/bm3d - Cheo Zhang results/out15000.tiff"
    
    img = cv2.imread(image_path) #image to load
    
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    #================================== Parameters initialization ==================================
    
    sigma = 3.5 # variance of the noise (change 25 to 3.5)
    
    lamb2d = 2.0
    
    lamb3d = 2.7
    
    Step1_ThreDist = 15000 # threshold distance (changed 2500 to 1500)
    ...
    cv2.imwrite(bm3d_img, final_img)
    
    if cv2.imwrite(bm3d_img, final_img) == True:
